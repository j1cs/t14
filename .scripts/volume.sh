...
function volume_notification {
    ...
    bar=`get_bar`
   exec $notify -R $id -u normal -i $vol_icon $bar
}
...
function mute_notification {
    ...
    if [ $muted == 'yes' ]
    then
        exec $notify -R $id -u normal -i $icon_mute `get_mute_bar`
    else
        exec $notify -R $id -u normal -i ${icon_path}`get_volume_icon $(get_volume)` `get_bar`
    fi
}
